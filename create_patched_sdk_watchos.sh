...
use_simulator="$1"
device_support_dir="$HOME/Library/Developer/Xcode/watchOS DeviceSupport/"
...
xcode_sim_runtime_path="$xcode_developer_path/Platforms/WatchOS.platform/Library/Developer/CoreSimulator/Profiles/Runtimes/watchOS.simruntime/Contents/Resources/RuntimeRoot"
if [ ! -d $xcode_sim_runtime_path ]; then
    xcode_sim_runtime_path="$xcode_developer_path/Platforms/WatchOS.platform/Developer/Library/CoreSimulator/Profiles/Runtimes/watchOS.simruntime/Contents/Resources/RuntimeRoot"
fi
xcode_default_sdk_path="$xcode_developer_path/Platforms/WatchOS.platform/Developer/SDKs/WatchOS.sdk"

xcode_sdk_paths="$xcode_developer_path/Platforms/WatchOS.platform/Developer/SDKs/*"
...
if [ -d "$device_support_dir" ] && [ "$use_simulator" == "-" ]; then
    for symbols_path in "$device_support_dir"*; do
        ...
        ios_version=${symbols_path_basename_array[1]}
        sdk_name=$(printf "WatchOS%s.sdk" $ios_version)
